...
.pod-wrap {
  ...
  /* ↓ Tune these in devtools Styles panel */
  --curve-opacity-from: 1; /* top opacity    */
  --curve-opacity-to: 0; /* bottom opacity */
}
...
/* Shared transform for both fill and stroke — tune on .pod-wrap in devtools */
.pod-curve-fill,
.pod-curve-stroke {
  position: absolute;
  width: 100%;
...
/* CSS gradient fill, clipped to the curve shape via mask */
.pod-curve-fill {
  /* height must be explicit when using mask so the element has dimensions */
  aspect-ratio: 1161.5 / 682.05;
  background: linear-gradient(
    in oklch to bottom right,
    color-mix(
        in oklch,
        var(--color-from) calc(var(--curve-opacity-from) * 100%),
        oklch(from var(--color-from) l c h / 0)
      ) -43%,
    color-mix(
        in oklch,
        var(--color-from) calc(var(--curve-opacity-to) * 100%),
        oklch(from var(--color-from) l c h / 0)
      )
      66%
  );
